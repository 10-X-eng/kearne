...
| RISK-022 | Codex app-server changes or fails and couples AI to CAD state | Medium / High | schema drift, unsupported runtime, transcript IDs leak into document schemas | pinned version, generated conformance, replaceable adapter, `No AI`, revision anchoring | disable AI/runtime update; preserve all local engineering workflows |
...
| RISK-024 | Test maintenance grows linearly with implementation change | High / High | repeated fixture/setup rewrites and one assertion file per behavior | schema-driven generators, reference models, conformance enrollment, scenario data, shrink/replay | freeze feature breadth and repair the shared assurance abstraction |
