...
void main()
{
    ...
    float coverPixels = record.meta.x == 11u
        ? max(abs(record.shape.z) + record.shape.x * 0.5,
              abs(record.shape.w) + record.shape.y * 0.5) + 2.0
        : max(record.appearance.x, record.appearance.y) * 0.5 + 2.0;
    vec2 expansion = vec2(coverPixels * ubuf.cameraHigh.z);
    vec2 relative = mix(minimum - expansion, maximum + expansion,
    ...
    item += vec2(cosine * delta.x - sine * delta.y,
                -(sine * delta.x + cosine * delta.y)) / ubuf.cameraHigh.z;
